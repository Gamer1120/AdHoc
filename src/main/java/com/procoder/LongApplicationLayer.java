...
public class LongApplicationLayer implements AdhocApplication {
	 ...
	 */
	@Override
	public void sendText(InetAddress dest, String input) {
		byte[] sender = null;
		try {
			if (dest != null) {
		...
		byte[] packet = null;
		try {
			packet = generatePacket(new byte[] { PacketType.TEXT.toByte() },
					sender, input.getBytes(ENCODING));
	...
	public void sendImage(InetAddress dest, File input) {
		...
		byte[] packet = null;
		try {
			packet = generatePacket(new byte[] { PacketType.FILE.toByte() },
